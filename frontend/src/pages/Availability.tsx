...
import { Calendar as CalIcon, Hotel, Mail, MapPin, Search, Users, Wifi, Coffee, Waves } from "lucide-react";
...
const normalizeHotel = (raw: any): HotelResult => {
  ...
  const rawImages = [
    raw.coverImageUrl, raw.imageUrl, raw.mainImageUrl,
  ...
  const amenities: string[] = Array.isArray(raw.amenities)
    ...
    ? raw.amenities.split(",").map((a: string) => a.trim()).filter(Boolean)
    : [];
  ...
  return {
    ...
    location,
    images: rawImages.length > 0 ? rawImages.map(String) : [fallbackHotelImg],
    tag: raw.status === "ACTIVE" ? "Available" : raw.status || "Hotel",
    amenities,
  };
};
...
const GuestHotelShell = ({ children }: { children: React.ReactNode }) => (
    ...
    <header className="bg-background sticky top-0 z-40 shadow-soft">
      <div className="container flex items-center justify-between h-20">
        ...
        <nav className="hidden md:flex items-center gap-2 text-sm">
          ...
          <Link to="/guest/availability" className="flex items-center gap-2 px-4 py-2 rounded-full bg-secondary text-primary font-bold">
    ...
    <GuestHotelShell>
      <section className="rounded-3xl bg-gradient-hero p-8 md:p-10 mb-8 shadow-soft">
        <span className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-primary bg-card px-3 py-1.5 rounded-full mb-3">
          <CalIcon className="h-3 w-3" /> Find rooms
        </span>
        <h1 className="text-3xl md:text-5xl font-bold text-primary-deep leading-tight">Check Availability</h1>
        <p className="text-muted-foreground max-w-xl mt-3">Pick your dates and browse available stays as a guest.</p>
      </section>
      ...
      {!searched ? (
        <div className="bg-gradient-hero rounded-3xl p-10 text-center flex flex-col items-center gap-4 shadow-soft">
          <Penguin mood="planning" className="h-40 w-40" />
          <h3 className="text-xl font-extrabold text-primary-deep">Plan your perfect getaway</h3>
          <p className="text-muted-foreground max-w-md">
            Pick check-in / out dates above and we'll show you all available rooms for those nights.
          </p>
        </div>
      ) : loading ? (
                ...
                <div className="flex items-end justify-end pt-2">
                  <Button asChild className="rounded-full bg-primary hover:opacity-95 font-bold">
                    <Link
                      to="/login"
                      state={{
                        redirectTo: `/user/book/${h.id}`,
                        bookingDraft: checkIn && checkOut ? {
